...
def main():
    ...
    # 1. Update array size from 10 to 11
    content = content.replace(
        "constexpr std::array<BuiltinPalette, 10> kPalettes",
        "constexpr std::array<BuiltinPalette, 11> kPalettes"
    )

    # 2. Find ".name = \"Tokyo-Night\"" and insert Purple Haze BEFORE it
    marker = '.name = "Tokyo-Night"'
    idx = content.find(marker)
    if idx == -1:
        print("ERROR: Could not find Tokyo-Night marker")
        return

    # Find the start of the line containing Tokyo-Night
    line_start = content.rfind("\n", 0, idx) + 1

    # Purple Haze entry - 16-space indentation, double-brace wrapping
    # The entry closes with "},<newline>" and Tokyo-Night follows
    purple_haze = '''            {
                    .name = "Purple Haze",
                    .dark =
                        FixedPaletteMode{
...
'''
    ...
    content = content[:line_start] + purple_haze + content[line_start:]
